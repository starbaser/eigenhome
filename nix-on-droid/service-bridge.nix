...
#
# Reads systemd.user.services from eigenhome's systemd-bridge (if loaded) and:
...
#   d) Unsupported types    → warnings
{
  ...
  # Access systemd.user.services from eigenhome's bridge if loaded.
...
in {
  ...
  config = {
    ...
    xdg.data.files."eigenhome/nod-autostart.sh" = mkIf hasAutostart {
    ...
    _nodOneshotActivations = mkIf hasOneshot (
      ...
      (name: svc: let
        ...
        workDir = svc.Service.WorkingDirectory or null;
      in
        nameValuePair "eigenhomeOneshot-${name}" ''
          $VERBOSE_ECHO "eigenhome: running oneshot service ${name}"
          ${concatMapStringsSep "\n" (e: "export ${toString e}") envVars}
          ${optionalString (workDir != null) "cd \"${toString workDir}\""}
