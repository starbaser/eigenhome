...
# The systemd service feeds into systemd-bridge.nix for hjem translation.
{
  ...
  keyfile = pkgs.writeText "eigenhome-dconf.ini" keyfileText;

  loadScript = pkgs.writeShellScript "eigenhome-dconf-load" ''
    ${pkgs.dconf}/bin/dconf load / < ${keyfile}
  '';
in {
  config = mkIf (cfg.enable && cfg.settings != {}) {
    systemd.user.services.eigenhome-dconf-load = {
      Unit = {
        Description = "Apply dconf settings (eigenhome)";
